...
GENRES_CSV = os.path.join(BASE_DIR, "archivos", "genres_api.csv")
...
try:
    ...
    genres_data = pd.read_csv(GENRES_CSV)

    df_recommendation["id"] = df_recommendation["id"].astype(str)
    genres_data["id_original"] = genres_data["id_original"].astype(str)

    # Combinar datos de películas y géneros
    movies_with_genres = pd.merge(df_recommendation, genres_data, left_on="id", right_on="id_original", how="left")
...
def get_recommendations(request: RecommendationRequest):
    try:
        if request.title not in movies_with_genres["title"].values:
            raise HTTPException(status_code=404, detail=f"La película '{request.title}' no se encontró en la base de datos.")
        idx = movies_with_genres[movies_with_genres["title"] == request.title].index[0]
        ...
        if len(recommended_titles) < request.n:
            extra_titles = movies_with_genres[~movies_with_genres["title"].isin(recommended_titles + [request.title])]["title"].tolist()
...
def recomendacion(titulo: str):
    try:
        if titulo not in movies_with_genres["title"].values:
            raise HTTPException(status_code=404, detail=f"La película '{titulo}' no se encontró en la base de datos.")
        idx = movies_with_genres[movies_with_genres["title"] == titulo].index[0]
